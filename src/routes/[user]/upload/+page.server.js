    ...
    upload: async ({ request }) => {
        try {
            ...
            if(quietFile && quietFileExtension && userName){
                writeFileSync(`$lib/vtubers/${userName}/${groupName}-quiet.${quietFileExtension}`, Buffer.from(await quietFile?.arrayBuffer()));
                console.log("WROTE QUIET FILE!");
            }
            if(speakFile && speakFileExtension && userName){
                writeFileSync(`$lib/vtubers/${userName}/${groupName}-speak.${speakFileExtension}`, Buffer.from(await speakFile?.arrayBuffer()));
